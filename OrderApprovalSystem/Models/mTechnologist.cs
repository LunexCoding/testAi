...
namespace OrderApprovalSystem.Models
{
    public class mTechnologist : mBaseOrderApproval
    {
        ...
        /// <summary>
        /// Согласовывает текущий заказ от имени технолога и создаёт следующий шаг цепочки согласования.
        /// </summary>
        /// <remarks>
        /// Логика формирования иерархии (ParentID) нового шага:
        /// <list type="number">
        /// <item>
        ///   <b>IsRework = true (текущий шаг — доработка):</b>
        ///   Ищет самую верхнюю запись с отклонением для следующего получателя в цепочке ParentID.
        ///   Если такая запись найдена — новый шаг становится дочерним к ней.
        ///   Если не найдена — новый шаг берёт ParentID текущего шага (остаётся на том же уровне).
        /// </item>
        /// <item>
        ///   <b>IsRework = false (обычное согласование):</b>
        ///   Если текущий шаг уже внутри цикла доработки (ParentID != null) и следующий получатель —
        ///   тот, кто ранее отклонил, то новый шаг остаётся в его цикле.
        ///   Иначе — выходим на корневой уровень (ParentID = null).
        /// </item>
        /// </list>
        /// После создания нового шага, при необходимости, перестраивается иерархия:
        /// <list type="bullet">
        /// <item>При IsRework=true: текущий шаг или его родительский под-цикл переносится под новый шаг.</item>
        /// <item>При обнаружении циклической ссылки: новый шаг меняется местами с родителем (swap).</item>
        /// </list>
        /// </remarks>
