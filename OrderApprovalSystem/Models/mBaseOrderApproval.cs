...
namespace OrderApprovalSystem.Models
{
    ...
    int? parentId) // Добавили параметр для связи
        ...
        /// <summary>
        /// Находит самую верхнюю (первую в цепочке) запись, в которой указанный получатель
        /// отклонил согласование (Result = "Не согласовано").
        /// </summary>
        /// <remarks>
        /// Используется для корректного формирования иерархии дерева истории согласований:
        /// <list type="bullet">
        /// <item>При IsRework=true: определяет, к чьему циклу относить возвращаемый шаг.</item>
        /// <item>При обычном согласовании: проверяет, находимся ли мы внутри цикла доработки.</item>
        /// </list>
        /// Поднимается по цепочке ParentID от текущей записи вверх, сохраняя ID каждого найденного
        /// совпадения. Возвращает последнее найденное (самое верхнее), чтобы все возвраты к одному
        /// отклоняющему были на одном уровне дерева, а не создавали глубокую вложенность.
        /// </remarks>
        /// <param name="currentRecord">Текущая запись истории согласования, от которой начинается поиск.</param>
        /// <param name="targetRecipientName">Имя получателя, чья отклоняющая запись ищется.</param>
        /// <returns>
        /// ID самой верхней отклоняющей записи для <paramref name="targetRecipientName"/>,
        /// или <c>null</c> если такой записи нет в цепочке (означает выход на корневой уровень).
        /// </returns>
